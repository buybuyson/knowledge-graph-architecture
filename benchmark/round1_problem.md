# Blind benchmark: Graph-walk vs flat RAG on a Python codebase

You are an independent measurer. Follow the problem exactly and **return only the number table in Section 6**. No long explanations needed. No sample answer is provided — you find and measure it yourself.
...
1. Which module reads the `HF_TOKEN` environment variable?
2. Which modules depend directly (import) on `utils`?
3. Where in the code is the Whisper model resolved (edges of type `resolves`)?
4. Which LLM-related environment variables does `postprocess.py` read?
5. If `utils` is changed, which modules might be affected (all edges pointing to `utils`)?
...
Extracted via AST from the Section 4 codebase. Each edge has `evidence` = file:line.
...
- Each unit counts as `1 + floor((length-1)/6)` tokens (units >6 characters count extra).
- token_in = tokens of the ENTIRE context you loaded to answer (code/table/question/framing prompt).
